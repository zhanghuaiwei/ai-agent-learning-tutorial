...
- 通过 202 Job API 与后台 Worker 可靠执行解析、Embedding、评测等长任务；
- 基于证据回答设备维护问题并提供引用；
- 从自然语言故障描述中提取结构化信息；
- 查询设备信息、历史告警和相关工单；
- 生成检查步骤与工单草稿；
...
- 按目标 JD 选做 Text2SQL、多模态、A2A/Agent Skills 和 Agent Runtime 实验；

### 2.2 项目明确不做什么

- 不直接启动、停止或控制真实设备；
- 不把模型建议包装成经过认证的维修结论；
- 不让模型执行未经 AST/Policy 验证的任意 SQL，不让数据 Agent 获得写权限；
...
flowchart LR
    ...
    J -->|"签发身份与权限上下文"| U
    P -->|"受控 REST Tools"| J
    P -->|"统一模型请求"| G["LLM Gateway / Release Policy"]
    G -->|"Chat / Embedding API"| Q["千问 / DeepSeek"]
    P --> V["知识索引与向量存储"]
    P --> A["Agent 状态、Memory、Usage"]
    P --> R["Job DB / Outbox / Redis Queue"]
    R --> W["摄取与评测 Worker"]
    W --> V
    W --> G
...
- 202 Job API、Outbox、Queue Adapter 与后台 Worker；
- 平台无关 Eval Event、Agent Release 与 Artifact 契约；
- 按 JD 选做 Text2SQL、多模态、A2A/Skills 与 Sandbox Policy。
...
| Job 状态/幂等/Outbox | Python Service | SQLite + InMemory Queue | PostgreSQL + Redis/MQ |
| 短期 Cache/Queue Delivery | 基础设施 | InMemory Fake | Redis/企业 MQ；不作为最终业务真相 |
| 原始文件/多模态派生/Artifact | Python Service | 本地测试目录 | 对象存储 + 短期签名 URL |
| Agent Release/Prompt/Tool 版本 | Python Service/控制面 | 文件 + Git SHA | 不可变 Release Snapshot |
...
│       ├── rag/                 # 摄取、检索、重排、引用
│       ├── data_agent/          # Text2SQL 计划、Schema、Policy
│       ├── media/               # OCR/VLM/ASR 与 Asset 血缘
│       ├── graphs/              # LangGraph 状态与节点
│       ├── memory/              # Checkpoint 与长期记忆
│       ├── evals/               # 数据集、Evaluator、报告
│       ├── jobs/                # Job 契约、Outbox、Queue 与 Worker
│       ├── observability/       # Trace、Metrics、Usage
│       ├── runtime/             # Release、Task、Artifact、Policy
│       ├── protocols/           # MCP/A2A Adapter 与 Skill 元数据
...
Dify 只用于同流程对照实验：平台通过服务端 Adapter 接入，不成为业务权限和工单事务的权威边界。若选型证据证明平台适合某个低风险只读流程，可以保留；核心代码仍需可测试、可迁移。

### 6.1 G9 岗位适配增量

这些模块不要求同时开启，也不改变主项目的业务边界：

| 专题 | 项目增量 | 关键证据 |
| --- | --- | --- |
| 框架迁移 | 同一诊断契约迁到两个目标框架 | 统一测试、评分表和 ADR |
| Text2SQL | 只读告警/工单统计子图 | AST Policy、租户隔离、执行评测 |
| 多模态 | 手册 OCR、仪表图片或语音描述 | 异步 Job、区域/时间戳引用、删除 |
| A2A/Skills | 远端备件查询 + 告警分诊 Skill | Task/Artifact、供应链与安全用例 |
| Agent Runtime | Release/Task/Gateway/Sandbox 模拟 | 控制面/数据面、容量与威胁模型 |
...
前端展示安全进度事件，而不是模型思维链。事件全集如下——核心四事件（`run.started` / `message.delta` / `run.completed` / `run.failed`）在第 2 阶段第 4 章首先落地，其余在 LangChain 与 Agent 阶段扩展：

```text
run.started
input.required            # 需要用户补充信息
run.progress              # 稳定业务阶段（如"检索中"），不是内部节点名
message.delta             # 文本增量
tool.started
tool.completed
tool.failed
